...
- v3.39 (`e72189f`): iteration 5 paper conformance audit — replaced two fatal `anyhow::bail!` admission controls in `main.rs` with warn-and-proceed best-effort degradation per Section 4.2 (phi-level budget checks at `phi.rs:358-377` already degrade individual branches gracefully at runtime), extracted hardcoded `.take(50)` pair truncation in `reduce_pairwise_top` to named constant `MAX_PAIRWISE_COMPARISONS`. Remaining flagged items are engineering safety guards (MAX_PHI_INPUT_BYTES, max_depth, Auto fallback, plan caching) that do not contradict the paper.
- v3.38 (`5b773b8`): iteration 4 paper conformance audit — inject explicit `[degraded:]` markers into `indexed_results` for failed children at depth>0 (preserves Map output cardinality for reduce operators instead of silent omission), reject empty plan checksums in `verify_checksum` (force recompute instead of accepting unvalidated legacy plans), lower Oracle continuation-skip threshold from hardcoded 1024 to 512 tokens so user-controlled analytical budgets get continuation attempts. Two critical deviations from iteration 3 report were false positives (phi.rs recursive case is complete, comb_cross is wrapped in neural comparison at depth-0 in reduce_pairwise_top).
- v3.37 (`154ef56`): iteration 3 paper conformance audit — added missing `filter_nonempty` guard to `reduce_summarise` so empty and `[degraded:]` child results are filtered before LLM synthesis, matching the pattern used by all other reducers (`reduce_multi_hop`, `reduce_search`, `reduce_aggregate`, `reduce_pairwise`). All other flagged deviations were false positives from truncated file access during iteration 1 analysis (verify.rs Pairwise arm is complete, phi.rs recursive structure is intact, keyword pre-filter is present at phi.rs:340, cost model recurrence is correct).
- v3.36 (`caed383`): iteration 2 paper conformance audit — removed unreachable dead branch in `reduce_search` (depth-0 multi-result path was already routed to `reduce_search_top` FilterBest by dispatch), confirmed all critical deviations from iteration 1 were resolved (FilterBest dispatch, neural pairwise at depth-0, question threading at intermediate reduce depths, `cfg.max_tokens` propagation replacing hardcoded 2048, exhaustive `TaskType` match in verifier, symbolic-only classify reduce).
- v3.35 (`cfacceb`): fixed three paper conformance deviations — removed artificial `.min(4096)` cap on leaf oracle token budgets so `cfg.max_tokens` propagates fully per the paper's token budget specification, corrected the cost model recurrence formula from `(n·k/τ - 1)/(k-1)` to `(n/τ - 1)/(k-1)` which was overestimating reduce costs by a factor of `k`, and introduced `ReduceNeuralType` enum to correctly classify Search (FilterBest) and Pairwise (neural comparison) as depth-0-only neural reduces instead of misclassifying them as zero-cost symbolic.
