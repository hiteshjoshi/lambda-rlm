...
| Direct async provider methods over boxed trait futures | Less runtime-swappable abstraction for providers | Eliminates per-call `Box::pin` allocations in hot call paths and reduces allocator fragmentation under load |
...
| Semaphores over channel buffering for backpressure | Callers can block instead of queuing unboundedly | Hard ceilings on recursion, LLM, and FD resources prevent OOM and descriptor exhaustion |
